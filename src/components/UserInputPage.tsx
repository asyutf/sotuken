        ...
        <div style={cardStyle}>
          ...
          {choresArray.fields.map((chore, i) => (
            <div key={chore.id} style={choreCardStyle}>
              ...
              <label style={labelStyle}>理想頻度</label>
              <select {...register(`chores.${i}.idealFrequency`, { valueAsNumber: true })} style={inputStyle}>
                {[...Array(14)].map((_, idx) => (
                  <option key={idx + 1} value={idx + 1}>{idx + 1} 日ごと</option>
                ))}
              </select>
              <label style={labelStyle}>最終実施日</label>
              <input type="date" {...register(`chores.${i}.lastPerformedDate`)} required style={inputStyle} />
